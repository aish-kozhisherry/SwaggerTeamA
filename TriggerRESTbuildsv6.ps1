...
{
  $name=$temp[$i]
  echo "this is the $ith file :$name"
  $action=$name.SubString(0,1)
  echo "this is the action on the swagger file: $action"
  $length=$name.Length
  echo "length of total file : $length"
  $filename=$name.SubString(1,$length-1)
  echo "this is filename substring $filename"
  if ($action -eq "D")
  {
    echo "A file was deleted and it will not pass through the pipeline. Filename: $filename"  
  } 
  else
  {
    $outputPath = "$filename"
    ...
    '
     ...
     while($status1 -ne "completed")
		  ...
		  {
					echo "Swagger-spec validation for $outputFile succeeded, now proceeding to Get Proxyname"
					$body = '
                                { 
                                ...
                                while($status -ne "completed")
								 ...
								 if ($result -eq "succeeded") 
			                        {
						            echo "Get Proxyname for $outputFile succeeded, now proceeding to Convert the swagger spec into a Proxy"
                                    $body = '
                                        { 
                                        "parameters":  "{\"filename\":  \"'+$outputFile+'\"}",
                                        "definition": {
                                        ...
                                        while($status -ne "completed")
                                            ...
                                            Write-Host "result" + $result
											if ($result -eq "succeeded") 
											{ echo "Convert to Proxy for $outputFile succeeded"}
											elseif ($result -eq "failed") {echo "Convert to Proxy for $outputFile failed"}
											else {echo "Convert to Proxy for $outputFile is In progress"}
                                            }
        
                                    }
								 elseif ($result -eq "failed")  { echo "Get proxyname for $outputFile  failed" }
